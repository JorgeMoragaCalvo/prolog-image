...
%pixelFlipH(Height, PixelIn, PixelOut):- pixbit(X, Y, Bit, Depth, PixelIn), 
    %(Y =< Height - 1 -> NewY = Y - 1; NewY = Y + 1), pixbit(X, NewY, Bit, Depth, PixelOut).
%    (Y < Height - 1 -> NewY is Y + 1; NewY is Y - 1), pixbit(X, NewY, Bit, Depth, PixelOut).
